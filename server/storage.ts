...
export class MongoStorage implements IStorage {
  ...
  async createMeeting(meetingData: CreateMeetingDto): Promise<Meeting> {
    const meeting = this.getEm().create(Meeting, {
      ...meetingData,
      userId: meetingData.user._id,
    });
  ...
  async fetchAndSyncBotMeetings(): Promise<Meeting[]> {
    try {
      const response = await fetch("http://localhost:3000/api/v1/bots/all");
      if (!response.ok) {
        throw new Error(`Failed to fetch bot meetings: ${response.statusText}`);
      }
      ...
      for (const botMeeting of botMeetings) {
        try {
          ...
          console.error(`Failed to sync bot meeting ${botMeeting._id}:`, error);
        }
      }
