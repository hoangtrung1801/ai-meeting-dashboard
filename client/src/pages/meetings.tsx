...
export default function Meetings() {
  ...
  const filteredMeetings = meetings && searchQuery 
  ...
  const { data: transcript } = useQuery({
    ...
    queryFn: () => fetch(`/api/meetings/${selectedMeetingId}/transcript`).then(res => res.json()),
  ...
  const { data: summary } = useQuery({
    ...
    queryFn: () => fetch(`/api/meetings/${selectedMeetingId}/summary`).then(res => res.json()),
    enabled: !!selectedMeetingId,
  });
  ...
  const { data: actionItems } = useQuery({
    ...
    queryFn: () => fetch(`/api/meetings/${selectedMeetingId}/action-items`).then(res => res.json()),
    enabled: !!selectedMeetingId,
  });
    ...
    <main className="flex-1 relative overflow-y-auto focus:outline-none">
      ...
      <div className="py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-semibold text-dark">Meetings</h1>
        </div>
        ...
        <Tabs defaultValue="all" className="mb-6">
          ...
          <TabsContent value="scheduled" className="mt-6">
            <div className="text-center py-12">
              <p className="text-gray-500 mb-4">No upcoming scheduled meetings.</p>
              <Button 
                onClick={() => setIsNewMeetingModalOpen(true)}
              >
                Schedule a Meeting
